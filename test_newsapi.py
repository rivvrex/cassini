...
"""
NewsAPI Test Script
Tests NewsAPI functionality to ensure it provides sufficient events
"""
...
def test_newsapi():
    ...
    if not config.USE_NEWS_API:
        print("❌ NewsAPI is disabled in config")
        return False
    ...
    print(f"✅ NewsAPI enabled with key: {config.NEWS_API_KEY[:8]}...")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if total_results > 0:
                ...
                print("⚠️ NewsAPI returned 0 results")
        else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if total_results > 0:
                print("✅ Economic news query successful")
                ...
                print("⚠️ No economic news found")
                print("💡 This might be normal if there are no economic news today")

        else:
            print(f"❌ Economic news query failed: {response.status_code}")
            return False

    except Exception as e:
        print(f"❌ Economic news query error: {e}")
        return False
    ...
    if total_results > 0:
        ...
        print(f"✅ Found {total_results} total articles")
        ...
        print("⚠️ NewsAPI returned no results")
        print("💡 This might be normal for quiet news days")
        print("🔄 Consider using Forex Factory as fallback")
        return True  # Still return True as API is working

if __name__ == "__main__":
    success = test_newsapi()
    if success:
        print("\n✅ NewsAPI test completed successfully!")
    else:
        print("\n❌ NewsAPI test failed!")
